    ...
    try {
      if (req.user.role !== "company") {
        return res.json(
          customResponse(
            403,
            "Only companies are allowed to view their tenders."
          )
        );
      }
    ...
    try {
      ...
      if (amount < tenderRecord.cost) {
        return res.json(
          customResponse(
            400,
            "Bidding amount must be greater than or equal to the cost of the tender"
          )
        );
      }
  ...
  async getAllCategories(req, res, next) {
    try {
      ...
      const uniqueCategories = Array.from(
        new Set(categories.map((tender) => tender.category))
      );
